...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # initialize the new_H and new_W of output feature map
    ...
    for n in range(N):
        for f in range(F):
            # bias matrix, for adding bias
            conv_new = np.ones([H_out, W_out]) * b[f]
            for c in range(C):
                ...
                for i in range(H_out):
                    for j in range(W_out):
                        conv_new[i, j] += np.sum(
                            padded_x[(i * stride):(i * stride + HH), (j * stride):(j * stride + WW)] * w[f, c, :, :])
            out[n, f, :, :] = conv_new
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
